...
let savings_chart = new Chart(
    document.getElementById('overview-savings-account'),
    {
        ...
        data: {
            ...
            datasets: [
                {
                    label: trans('home.chart_evolution_of'),
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: trans('home.chart_evolution_of') + ': Example Account',
                    position: 'top'
                },
                legend: {
                    display: false
...
function set_operation_type_list() {
    ...
    xhr.onload = () => {
        if (Math.floor(xhr.status / 100) === 2) {
            ...
            pie_labels = [trans('home.chart_remains')];
            pie_colors = ["#36a2eb"];
            for (let i = 0; i < 9; i++) {
    ...
    xhr.onload = () => {
        if (Math.floor(xhr.status / 100) === 2) {
            ...
            savings_chart.options.plugins.title = {
                display: true,
                text: `${trans('home.chart_evolution_of')}: ${highest_account.label}`,
                position: 'top'
            };
    ...
    xhr.onload = () => {
        if (Math.floor(xhr.status / 100) === 2) {
            ...
            budget_chart.options.plugins.title = {
                display: true,
                text: `${trans('home.chart_monthly_budget_of')}: ${random_checking_account.label}`,
                position: 'left'
            };
